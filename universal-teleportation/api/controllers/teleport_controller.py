...
capture_manager_module = _load_module(
	"capture_manager", os.path.join(PROJECT_DIR, "state-capture", "capture_manager.py")
)
snapshot_builder_module = _load_module(
	"snapshot_builder", os.path.join(PROJECT_DIR, "snapshot-engine", "snapshot_builder.py")
)
transfer_manager_module = _load_module(
	"transfer_manager", os.path.join(PROJECT_DIR, "transfer-layer", "transfer_manager.py")
)
telemetry_module = _load_module(
	"node_telemetry", os.path.join(PROJECT_DIR, "monitoring", "node_telemetry.py")
)
runtime_dispatcher_module = _load_module(
    "runtime_dispatcher", os.path.join(PROJECT_DIR, "runtime_dispatcher.py")
)
...
container_adapter_module = _load_module(
	"container_adapter", os.path.join(PROJECT_DIR, "runtime-adapters", "container_adapter.py")
)
container_checkpoint_engine_module = _load_module(
	"container_checkpoint_engine", os.path.join(PROJECT_DIR, "snapshot-engine", "container_checkpoint.py")
)
container_restore_engine_module = _load_module(
	"container_restore_engine", os.path.join(PROJECT_DIR, "state-reconstruction", "container_restore.py")
)
...
class TeleportController:
	def __init__(self):
		self.capture_manager = CaptureManager(snapshot_dir=os.path.join(PROJECT_DIR, "snapshots"))
		self.transfer_manager = TransferManager(snapshot_dir=os.path.join(PROJECT_DIR, "snapshots"))
		...
		self.container_checkpoint = ContainerCheckpointEngine(snapshot_dir=os.path.join(PROJECT_DIR, "snapshots"))
		self.container_restore = ContainerRestoreEngine(snapshot_dir=os.path.join(PROJECT_DIR, "snapshots"))
	...
	def teleport_remote(self, process_id, target_node_id, target_os, protocol="auto"):
		...
		source_dir = os.path.join(PROJECT_DIR, "snapshots", f"process_{process_id}")
		artifact_name = f"remote_{tracking_id}.tar.gz"
		artifact_path = os.path.join(PROJECT_DIR, "snapshots", artifact_name)
		...
		transfer_result = self.transfer_manager.send_snapshot(
			process_id=process_id,
			target_path=os.path.join(PROJECT_DIR, "snapshots", "remote-inbox"),
	...
	def checkpoint_container(self, container_id, runtime="auto", checkpoint_name="uat-checkpoint"):
		...
		if not checkpoint_info:
			...
			checkpoint_path = os.path.join(PROJECT_DIR, "snapshots", checkpoint_name)
